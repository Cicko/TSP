...
namespace GOMA {
    ...
    bitset_128_t& bitset_128_t::operator= (const bitset_128_t& bs)
    {
        if (block_)
            delete [] block_;
	...
	bool bitset_128_t::contains(int i) {
       ...
       // assert(i > 0);
        ...
        return test_bit_64(block_, i - 1) != 0;
#ifdef _DEBUG
        assert(i < 128);
        assert(i >=  0);
#endif        
	...
	bool bitset_128_t::empty_intersec(const bitset_128_t& bs) {
	...
	#ifdef _DEBUG
	ostream& bitset_128_t::write(ostream& os) {
	...
	bool bitset_128_t::operator==(const bitset_128_t& bs) {
